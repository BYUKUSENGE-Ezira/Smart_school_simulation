...
static void publish_mqtt_status_if_due(void)
{
    ...
    static bool previous_status_valid =
        false;

    static bool previous_auto_enabled =
        false;

    static bool previous_alarm_active =
        false;

    static bool previous_pa_active =
        false;

    static bool previous_bell_blocked =
        false;

    static char previous_announcement_state[20] =
        "";

    if (!mqtt_manager_is_connected())
    {
        /*
         * Publish a fresh snapshot immediately after the
         * next MQTT connection.
         */
        previous_status_valid = false;
        return;
    }
    ...
    /*
     * Critical changes must not wait for the normal
     * ten-second heartbeat.
     */
    bool critical_state_changed =
        !previous_status_valid ||

        web_status.auto_enabled !=
            previous_auto_enabled ||

        web_status.alarm_active !=
            previous_alarm_active ||

        web_status.announcement_pa_active !=
            previous_pa_active ||

        web_status.bell_blocked_by_announcement !=
            previous_bell_blocked ||

        strcmp(
            web_status.announcement_state,
            previous_announcement_state
        ) != 0;
    ...
    mqtt_status = (mqtt_alarm_status_t){
        ...
        .announcement_pa_active =
            web_status.announcement_pa_active,

        .bell_blocked_by_announcement =
            web_status.bell_blocked_by_announcement,
    ...
    snprintf(
        mqtt_status.announcement_state,
        sizeof(mqtt_status.announcement_state),
        "%s",
        web_status.announcement_state
    );
    ...
    snprintf(
        previous_announcement_state,
        sizeof(previous_announcement_state),
        "%s",
        web_status.announcement_state
    );
}
